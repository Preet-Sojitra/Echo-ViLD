...
OUTPUT_DIR = PROJECT_ROOT / "sam_peav_outputs"
...
def load_peav():
    ...
    processor = PeAudioVideoProcessor.from_pretrained(str(PEAV_MODEL_PATH))
...
def process_single_image(
    ...
    save_embeddings_json(
        OUTPUT_DIR / f"{image_id}_embeddings.json",
        baseline_embeds,
        sam_nocontext_embeds,
        sam_withcontext_equal_embeds,
        sam_withcontext_80_20_embeds,
    ...
    save_metadata_json(
        OUTPUT_DIR / f"{image_id}_metadata.json",
        metadata
    )
